          ...
          <el-form :model="taskForm" label-position="top">
            ...
            <el-form-item :label="$t('uiAutomation.ai.walletMode')">
              <el-switch
                ...
                :active-text="$t('uiAutomation.ai.walletModeEnabled')"
                :inactive-text="$t('uiAutomation.ai.walletModeDisabled')"
              />
            </el-form-item>

            <transition name="el-fade-in-linear">
              <div v-if="taskForm.walletMode" class="wallet-panel">
                <el-alert
                  :title="$t('uiAutomation.ai.walletModeTip')"
                  type="info"
                  :closable="false"
                  show-icon
                  class="wallet-tip"
                />
                <el-alert
                  title="当前钱包自动化仅支持 Chrome + MetaMask 全页模式；Edge 或 Popup 模式会在预检查阶段直接失败。"
                  type="warning"
                  :closable="false"
                  show-icon
                  class="wallet-tip"
                ...
                <el-form-item :label="$t('uiAutomation.ai.walletProvider')" required>
                  <el-select v-model="taskForm.walletProvider" class="wallet-select">
                    <el-option :label="$t('uiAutomation.ai.walletProviders.metamask')" value="metamask" />
                  </el-select>
                </el-form-item>

                <el-form-item :label="$t('uiAutomation.ai.walletTargetChain')" required>
                  <el-select
                  ...
                  >
                    <el-option
                      v-for="chain in walletChainOptions"
                      :key="chain.value"
                      :label="chain.label"
                      :value="chain.value"
                    />
                ...
                <el-form-item :label="$t('uiAutomation.ai.walletForceCloseExistingChrome')">
                  <el-switch
...
const taskForm = reactive({
  ...
  walletTargetChain: 'Ethereum Mainnet',
  walletForceCloseExistingChrome: true
})

const walletChainOptions = computed(() => [
  { value: 'Ethereum Mainnet', label: t('uiAutomation.ai.walletChains.ethereumMainnet') },
  { value: 'Sepolia', label: t('uiAutomation.ai.walletChains.sepolia') },
  { value: 'BNB Chain', label: t('uiAutomation.ai.walletChains.bnbChain') },
  { value: 'Polygon', label: t('uiAutomation.ai.walletChains.polygon') },
  { value: 'Arbitrum One', label: t('uiAutomation.ai.walletChains.arbitrumOne') },
  { value: 'Base', label: t('uiAutomation.ai.walletChains.base') }
])
...
const handleRun = async () => {
  if (taskForm.walletMode) {
    if (!taskForm.walletProvider) {
      ElMessage.warning(t('uiAutomation.ai.walletMessages.providerRequired'))
      return
    }

    if (!taskForm.walletTargetChain) {
      ElMessage.warning(t('uiAutomation.ai.walletMessages.targetChainRequired'))
      return
    }
  }
  ...
  try {
    const response = await runAdhocAITask({
      ...
      wallet_target_chain: taskForm.walletMode ? taskForm.walletTargetChain : '',
      wallet_force_close_existing_chrome: taskForm.walletMode ? taskForm.walletForceCloseExistingChrome : false
    })
