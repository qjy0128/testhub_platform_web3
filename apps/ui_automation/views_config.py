...
from .wallet_session import (
    is_default_chrome_user_data_dir,
    is_supported_chrome_executable,
    prepare_wallet_browser_session,
    probe_wallet_cdp_capabilities,
    resolve_metamask_extension_id,
)

logger = logging.getLogger(__name__)


def run_wallet_runtime_preflight(wallet_context):
    from .ai_base import probe_metamask_wallet_runtime
...
class AIIntelligentModeConfigViewSet(viewsets.ViewSet):
    ...
    def _perform_model_connection_test(self, *, provider, base_url, api_key, model_name):
        ...
        url = self._build_chat_completions_url(resolved_base_url)
        headers = {
        ...
        try:
            ...
            logger.error(f"AI智能模式 - API连接测试超时: {repr(exc)}")
            return Response(
                {'error': '连接测试超时: 请检查网络连接或API地址是否正确'},
                status=status.HTTP_408_REQUEST_TIMEOUT
            )
        except Exception as exc:
            logger.error(f"AI智能模式 - API连接测试异常: {repr(exc)}")
            return Response(
                {'error': f'连接异常: {str(exc)}'},
    ...
    def test_connection_preview(self, request):
        ...
        return self._perform_model_connection_test(
            provider=provider,
            base_url=base_url,
            api_key=api_key,
...
class WalletBrowserConfigViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        configs = self.queryset.order_by('-updated_at')
        serializer = WalletBrowserConfigSerializer(configs, many=True)
        return Response(serializer.data)

    def create(self, request):
        serializer = WalletBrowserConfigSerializer(data=request.data, context={'request': request})
        serializer.is_valid(raise_exception=True)
        config = serializer.save()
        return Response(WalletBrowserConfigSerializer(config).data, status=status.HTTP_201_CREATED)

    def retrieve(self, request, pk=None):
        config = self.queryset.filter(pk=pk).first()
        if not config:
            return Response({'error': 'Wallet config not found'}, status=status.HTTP_404_NOT_FOUND)
        return Response(WalletBrowserConfigSerializer(config).data)
    ...
    def update(self, request, pk=None):
        ...
        serializer = WalletBrowserConfigSerializer(
            config,
            data=request.data,
    ...
    @action(detail=False, methods=['post'], url_path='prepare_session')
    def prepare_session(self, request):
        force_close_existing_chrome = request.data.get('force_close_existing_chrome')
        if not isinstance(force_close_existing_chrome, bool) and force_close_existing_chrome is not None:
            force_close_existing_chrome = str(force_close_existing_chrome).strip().lower() in {'1', 'true', 'yes', 'on'}

        try:
            session = prepare_wallet_browser_session(
        ...
        wallet_context = {
            ...
            'runtime_user_data_dir': session.runtime_user_data_dir,
            'profile_directory': session.profile_directory,
        }
        preflight = run_wallet_runtime_preflight(wallet_context)
        ...
        return Response({
            ...
            'cdp_connected': preflight.get('cdp_connected', False),
            'extension_pages_visible': preflight.get('extension_pages_visible', False),
            'supported': preflight.get('supported', False),
            'unsupported_reason': preflight.get('unsupported_reason', ''),
            'pages': preflight.get('pages', []),
        })

    def _legacy_test_connection(self, request, pk=None):
        """
        测试已保存配置的连接
        """
        try:
    ...
    def test_connection(self, request, pk=None):
        ...
        chrome_executable_exists = Path(config.chrome_executable_path).exists()
        browser_supported = chrome_executable_exists and is_supported_chrome_executable(
            config.chrome_executable_path
        )
        user_data_dir_exists = Path(config.user_data_dir).exists()
        uses_default_chrome_user_data_dir = is_default_chrome_user_data_dir(config.user_data_dir)
        metamask_extension_id = ''
        errors = []

        try:
            metamask_extension_id = resolve_metamask_extension_id(config)
        except Exception as exc:
            errors.append(str(exc))

        if not chrome_executable_exists:
            errors.append(f'Chrome executable not found: {config.chrome_executable_path}')
        elif not browser_supported:
            errors.append('Only Google Chrome is supported for wallet automation. Edge popup mode is not supported.')
        if not user_data_dir_exists:
            errors.append(f'Chrome user data dir not found: {config.user_data_dir}')
        if config.launch_mode == 'global_profile' and uses_default_chrome_user_data_dir:
            errors.append(
                'global_profile 模式不能直接附加默认 Chrome 用户数据目录，请改用 runtime_clone 或使用非默认 profile 目录。'
            )
        ...
        if metamask_extension_id:
            cdp_capabilities = probe_wallet_cdp_capabilities(
                debugger_address=f'127.0.0.1:{config.remote_debugging_port}',
                remote_debugging_port=config.remote_debugging_port,
                metamask_extension_id=metamask_extension_id,
            )

        supported = (
            not errors
            and browser_supported
            and cdp_capabilities.get('cdp_connected', False)
            and cdp_capabilities.get('extension_pages_visible', False)
        )

        payload = {
            'message': '配置有效' if not errors else '配置校验失败',
            'wallet_provider': config.wallet_provider,
            'launch_mode': config.launch_mode,
            'remote_debugging_port': config.remote_debugging_port,
            'chrome_executable_exists': chrome_executable_exists,
            'browser_supported': browser_supported,
            'user_data_dir_exists': user_data_dir_exists,
            'uses_default_chrome_user_data_dir': uses_default_chrome_user_data_dir,
            'metamask_extension_id': metamask_extension_id,
            'cdp_connected': cdp_capabilities.get('cdp_connected', False),
            'extension_pages_visible': cdp_capabilities.get('extension_pages_visible', False),
            'extension_page_urls': cdp_capabilities.get('extension_page_urls', []),
            'supported': supported,
            'unsupported_reason': cdp_capabilities.get('unsupported_reason', ''),
        }
        if errors:
