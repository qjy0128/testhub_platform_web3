...
class WalletSession(models.Model):
    ...
    launch_mode = models.CharField(
        max_length=50,
        choices=WalletBrowserConfig.LAUNCH_MODE_CHOICES,
        default='runtime_clone',
        verbose_name='启动模式',
    )
    process_id = models.PositiveIntegerField(null=True, blank=True, verbose_name='Chrome 进程 ID')
    remote_debugging_port = models.PositiveIntegerField(default=9222, verbose_name='远程调试端口')
    runtime_user_data_dir = models.CharField(max_length=500, blank=True, default='', verbose_name='Wallet runtime user data dir')
    ...
    def __str__(self):
        return f'{self.wallet_provider} - {self.status}'
...
class WalletActionLog(models.Model):
    ...
    def __str__(self):
        return f'{self.action_name} - {self.action_status}'
